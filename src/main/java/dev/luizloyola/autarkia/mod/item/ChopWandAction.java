 ...
 * resolved trunk to {@link FellTree}.
 *
 ...
 * perception. Anything that is not a tree, or is in another dimension, is not claimed at all, and
 * the click falls through to "walk there".
 */
    ...
    public Optional<Component> useOn(Click click) {
        ...
        for (GrownRegion.Part tree : treesAround(probe, seed, kind, person)) {
            ...
            boolean autoDisabled = person.brain().run(
                    new FellTree(Anchors.choose(tree.approach(), seed)));
