...
public final class ChopKnownTree implements Method {
    ...
    public List<Task> decompose(BrainContext ctx) {
        ...
        ctx.claims().claim(Pois.TREE, memory.anchor(), ctx.percepts().time());
    ...
    private static Optional<PoiMemory> nearest(BrainContext ctx) {
        ...
        for (PoiMemory m : ctx.knowledge().all(Pois.TREE)) {
            if (ctx.knowledge().isAvoided(Pois.TREE, m.anchor(), now)
                    || !ctx.claims().availableTo(Pois.TREE, m.anchor(), now)) {
                continue;
            }
